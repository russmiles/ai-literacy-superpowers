...
- It does _not_ write to `AGENTS.md`, `REFLECTION_LOG.md`, or `ONBOARDING.md`.
  `AGENTS.md` and `REFLECTION_LOG.md` are curated by humans; `ONBOARDING.md`
  is regenerated by `/harness-onboarding` under the user's deliberate trigger.
...
3. **HARNESS.md Status section accuracy** — inline the Status block update.
   Read the current constraint and GC enforcement counts from HARNESS.md,
   compute the new values, then rewrite ONLY the four lines under the
   `<!-- Auto-updated by /harness-audit — do not edit manually -->` marker
   (`Last audit:`, `Constraints enforced:`, `Garbage collection active:`,
   `Drift detected:`). The "Drift detected:" value summarises the findings
   from this sync run — `no` if every selected `[auto]` action returned
   `in sync`, otherwise `yes (<short summary>)`.

   Do NOT shell out to `/harness-audit`. The full audit also writes the
   README badge and runs heavy constraint regression scans, both outside
   sync's auto scope. Users who want a full audit run it separately.
...
- `.windsurf/rules/**`
- `observability/snapshots/**` — only when `/harness-health` was a
  selected `[auto]` action (snapshot creation).
- `HARNESS.md` — only when the diff is scoped to the Status block (the
  four lines under the `<!-- Auto-updated by /harness-audit — do not
  edit manually -->` marker). Mutations anywhere else in HARNESS.md
  are out of bounds.

For HARNESS.md, run an additional scoped-diff check: parse
`git diff --cached HARNESS.md` and verify every changed hunk is
contained within the four-line Status block region. Any HARNESS.md
hunk that touches lines outside that region is a trust-boundary
violation.

`AGENTS.md`, `REFLECTION_LOG.md`, and `ONBOARDING.md` are never on the
allow-list — sync surfaces `ONBOARDING.md` drift as a `[manual]`
finding but never writes to it.

If any path falls outside the allow-list (or any HARNESS.md change
escapes the Status block region), refuse to commit. Tell the user:
...
>
> Unexpected path(s) or out-of-scope HARNESS.md hunk(s):
> `[list each offending path or line range]`
...
Stage every allow-listed surface that may have been touched:
...
Git only stages files with actual changes; paths that were not touched are safe
to include in the list.

Commit message describes which surfaces were synced. Suggested defaults:

- Convention files only: `"chore: sync convention files to HARNESS.md"`
- HARNESS.md Status block only: `"chore: sync HARNESS.md Status section via /harness-sync"`
- Snapshot regen only: `"chore: refresh harness-health snapshot via /harness-sync"`
- Mixed: `"chore: sync surfaces to HARNESS.md"` (with a body listing what changed)
...
git add HARNESS.md \
        ...
        .windsurf/rules/ \
        observability/snapshots/
git commit -m "<message describing which surfaces were synced>"
```
...
| Pre-commit guard rejects out-of-bounds path (`AGENTS.md`, `REFLECTION_LOG.md`, `ONBOARDING.md`) or a HARNESS.md change outside the Status block | Refuse to commit; treat as bug in underlying primitive |
